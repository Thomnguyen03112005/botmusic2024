...
import { EmbedBuilders, ComponentBuilder } from "../../Handlers/functions.js";
...
const slashCommand = new slashCommandBuilder({
  ...
  description: "Một số lệnh về thông tin của bot", // Mô tả lệnh
  ...
  try {
    if (interaction.options.getSubcommand() === "avatar") {
      ...
      const embeds = new EmbedBuilders({
        title: { name: '🖼️ Avatar', url: avatar },
        colors: "Random",
        timestamp: new Date().now(),
        images: avatar
      });

      const createButton = new ComponentBuilder([
        {
          type: "ButtonBuilder",
          options: [
            { label: "PNG", style: ButtonStyle.Link, url: member.displayAvatarURL({ format: 'png', size: 1024 }) }
          ]
        }
      ]);
